...
def parse_opt():
    ...
    parser.add_argument('--do_set1', default=False, action="store_true", help="whether to train or test the model")
    parser.add_argument('--do_balanced', default=False, action="store_true", help="whether to train or test the model")
    ...
    parser.add_argument('--gqa_loader', type=str, default="hazard_detection", help="whether to train or test the model")
...
if __name__ == "__main__":
    ...
    with open('./meta_info/full_vocab.json', 'r') as f:
        vocab = json.load(f)
        ivocab = {v: k for k, v in vocab.items()}

    with open('./meta_info/answer_vocab.json', 'r') as f:
        answer = json.load(f)
    ...
    if args.model == 'MCAN':
        ...
        from Networks.mnnm_models.soft_logic_transformer import *
    ...
    if args.do_train or args.do_train_all or args.do_finetune or args.do_set1 or args.do_balanced:
        if args.do_train_all:
            train_split = 'trainval_all_fully'
            print("using the all programs for bootstrapping")
        elif args.do_set1:
            train_split = 'set1'
            print("using set1 for bootstrapping")
        elif args.do_balanced:
            train_split = 'balanced_train'
            print("using balanced_train for bootstrapping")
        else:
        ...
        if args.gqa_loader == "v1":
            ...
            test_dataset = GQA_v3(split=testdev_split, mode='val', contained_weight=args.contained_weight,
                                  threshold=args.threshold, folder=args.data, cutoff=args.cutoff, **basic_kwargs)
        elif args.gqa_loader == 'hazard_detection':
            train_dataset = HD_datasets_v2(split=train_split, mode='train', contained_weight=args.contained_weight,
                                   threshold=args.threshold, folder=args.data, cutoff=args.cutoff, **basic_kwargs)
        ...
        for epoch in range(args.resume + 1, args.num_epochs):
            ...
            checkpoints_loc = np.argmin(global_maxacc)
            if acc > global_maxacc[checkpoints_loc]:
                global_maxacc[checkpoints_loc] = acc
                torch.save(model.state_dict(), os.path.join(repo, 'model_best_{}'.format(checkpoints_loc)))
            
            # if acc > global_maxacc:
            #     global_maxacc = acc
            #     torch.save(model.state_dict(), os.path.join(repo, 'model_best'))
            model.train()
