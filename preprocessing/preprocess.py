...
def create_inputs(splits, output):
    ...
    for split in splits:
        ...
        #        data = json.load(f)
        # else:
        if split == 'hazard_detection':
            with open('gqa_questions/{}_programs.json'.format(split)) as f:
                data = json.load(f)
...
def create_inputs_hazard_detection(splits, output):
    def find_all_nums(strings):
        # print(strings)
    ...
    for split in splits:
        ...
        if split == 'balanced_train':
            with open('../../datasets/hazard_detection/image_rule_pair/Rule124_triplet_balanced_train_answer_program_list.json') as f:
                data = json.load(f)
            print("loading data from {}".format(
                '../../datasets/hazard_detection/image_rule_pair/Rule124_triplet_balanced_train_answer_program_list.json'))
        elif split ==  'balanced_test':
            with open('../../datasets/hazard_detection/image_rule_pair/Rule124_triplet_balanced_test_answer_program_list.json') as f:
                data = json.load(f)
            print("loading data from {}".format(
                '../../datasets/hazard_detection/image_rule_pair/Rule124_triplet_balanced_test_answer_program_list.json'))
        else:
            with open('../../datasets/hazard_detection/image_rule_pair/Rule124_triplet_50_{}_answer_program_list.json'.format(split)) as f:
                data = json.load(f)
                print("loading {}".format('../../datasets/hazard_detection/image_rule_pair/Rule124_triplet_50_{}_answer_program_list.json'.format(split)))
        ...
        for idx, entry in enumerate(data):
            ...
            while len(depth) < len(programs):
                ...
                if len(tmp_depth) == 0 and len(tmp) == 0 and len(tmp_connection) == 0:
                    ...
                    depth.update(tmp_depth)
            # import pdb; pdb.set_trace()
            if len(entry['triplets']) == 1:
                rule_type = 1
            elif len(entry['triplets']) == 3:
                rule_type = 2
            elif len(entry['triplets']) == 2:
                rule_type = 4
            # results.append([entry[0], entry[1], returns, inputs, connection, entry[-2], entry[-1]])
            
            results.append([entry['ImageId'], entry['rule'], returns, inputs, connection, rule_type, entry['answer']])
            # print(results)
            # import pdb; pdb.set_trace()
            # entry[0]: n161313 image_id # entry[1]: rule # entry[-2]: rule type # entry[-1]:answer
            sys.stdout.write("finished {}/{} \r".format(idx, len(data)))
...
if arg == 'trainval_all':
    ...
    with open('/mntnfs/med_data2/ziyitang/VQA/Meta-Module-Network/gqa-questions/train_all_questions/train_all_questions_0.json') as f:
        raw_data.update(json.load(f)); print('> ', end='')
    with open('/mntnfs/med_data2/ziyitang/VQA/Meta-Module-Network/gqa-questions/train_all_questions/train_all_questions_1.json') as f:
        raw_data.update(json.load(f)); print('> ', end='')
    with open('/mntnfs/med_data2/ziyitang/VQA/Meta-Module-Network/gqa-questions/train_all_questions/train_all_questions_2.json') as f:
        raw_data.update(json.load(f)); print('> ', end='')
    with open('/mntnfs/med_data2/ziyitang/VQA/Meta-Module-Network/gqa-questions/train_all_questions/train_all_questions_3.json') as f:
        raw_data.update(json.load(f)); print('> ', end='')
    with open('/mntnfs/med_data2/ziyitang/VQA/Meta-Module-Network/gqa-questions/train_all_questions/train_all_questions_4.json') as f:
        raw_data.update(json.load(f)); print('> ', end='')
    with open('/mntnfs/med_data2/ziyitang/VQA/Meta-Module-Network/gqa-questions/train_all_questions/train_all_questions_5.json') as f:
        raw_data.update(json.load(f)); print('> ', end='')
    with open('/mntnfs/med_data2/ziyitang/VQA/Meta-Module-Network/gqa-questions/train_all_questions/train_all_questions_6.json') as f:
        raw_data.update(json.load(f)); print('> ', end='')
    with open('/mntnfs/med_data2/ziyitang/VQA/Meta-Module-Network/gqa-questions/train_all_questions/train_all_questions_7.json') as f:
        raw_data.update(json.load(f)); print('> ', end='')
    with open('/mntnfs/med_data2/ziyitang/VQA/Meta-Module-Network/gqa-questions/train_all_questions/train_all_questions_8.json') as f:
        raw_data.update(json.load(f)); print('> ', end='')
    with open('/mntnfs/med_data2/ziyitang/VQA/Meta-Module-Network/gqa-questions/train_all_questions/train_all_questions_9.json') as f:
        raw_data.update(json.load(f)); print('> ', end='')
    with open('/mntnfs/med_data2/ziyitang/VQA/Meta-Module-Network/gqa-questions/val_all_questions.json') as f:
        raw_data.update(json.load(f)); print('> finish loading')
    preprocess(raw_data, 'questions/trainval_all_programs.json')

elif arg == 'create_balanced_programs':
    with open('/mntnfs/med_data2/ziyitang/VQA/Meta-Module-Network/gqa-questions/train_balanced_questions.json') as f:
        raw_data = json.load(f)
    with open('/mntnfs/med_data2/ziyitang/VQA/Meta-Module-Network/gqa-questions/val_balanced_questions.json') as f:
        raw_data.update(json.load(f))
    preprocess(raw_data, 'questions/trainval_balanced_programs.json')
    with open('/mntnfs/med_data2/ziyitang/VQA/Meta-Module-Network/gqa-questions/testdev_balanced_questions.json') as f:
    ...
    create_inputs(['trainval_unbiased_fully'], 'gqa_questions/trainval_unbiased_fully_inputs.json')
    create_inputs(['testdev_pred'], 'gqa_questions/testdev_pred_inputs.json')
    ...
    save('meta_info/full_vocab.json', 'meta_info/en_emb.npy')
    #save('nl2prog/concept_vocab.json', 'models/concept_emb.npy')

elif arg == 'hazard_detection':
    # create_inputs_hazard_detection(['set1'], '/mntnfs/med_data5/shunlinlu/datasets/hazard_detection/image_rule_pair/Rule124_triplet_50_set1_inputs.json')
    # create_inputs_hazard_detection(['set2'], '/mntnfs/med_data5/shunlinlu/datasets/hazard_detection/image_rule_pair/Rule124_triplet_50_set2_inputs.json')
    # create_inputs_hazard_detection(['balanced_train'], '/mntnfs/med_data5/shunlinlu/datasets/hazard_detection/image_rule_pair/Rule124_triplet_balanced_train_inputs.json')
    create_inputs_hazard_detection(['balanced_test'], '../../datasets/hazard_detection/image_rule_pair/Rule124_triplet_balanced_test_inputs.json')
