...
def parse_opt():
    ...
    parser.add_argument('--stacking', type=int, default=2, help="whether to train or test the model")
    ...
    parser.add_argument('--load_from', type=str, default=r"D:\学习\数据集标注\demo\hazard_detection\models\TreeSparsePost2Full\model_best_0", help="whether to train or test the model")
    parser.add_argument('--data', type=str, default=r"D:\学习\数据集标注\demo\datasets\hazard_detection\roi_feats_after_processing",
                        help="whether to train or test the model")
...
def inference(img, txt):
    ...
    basic_kwargs = dict(length=args.length, object_info=os.path.join(args.meta, args.object_info),
                        ...
                        forbidden=args.forbidden, folder=args.data, threshold=args.threshold)


    if device == torch.device('cpu'):
        model.load_state_dict(torch.load(args.load_from, map_location=torch.device('cpu')))
    else:
    ...
    # import pdb; pdb.set_trace()
    test_dataset = single_image_text_test(**basic_kwargs)
    
    # img = "0000064.jpg"
    # txt = "No face mask"
    
    
    batch = test_dataset.forward(img, txt)
    model.eval()
    # for i in range(len(batch)):
    #     print(type(batch[i]))
    
    questionId = batch[-1]
    batch = tuple(Variable(torch.tensor(t)).unsqueeze(0).to(device) for t in batch[:-1])
    results = model(*batch[:-2])
    if isinstance(results, tuple):
        logits = results[1]
    else:
    ...
    return inv_answer[int(preds)]
    # print("result = {}".format(inv_answer[int(preds)]))
...
if __name__ == "__main__":
    ...
    print(inference(img=img, txt=txt))
    ...
    # with open(r'D:\学习\数据集标注\demo\hazard_detection\meta_info\full_vocab.json', 'r') as f:
    #     vocab = json.load(f)
    #     ivocab = {v: k for k, v in vocab.items()}


    # with open(r'D:\学习\数据集标注\demo\hazard_detection\meta_info\answer_vocab.json', 'r') as f:
    #     answer = json.load(f)
    #     inv_answer = {v: k for k, v in answer.items()}
